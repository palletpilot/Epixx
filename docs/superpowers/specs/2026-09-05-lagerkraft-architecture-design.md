...
2. **Catalog**: articles, packaging levels (each/package/pallet), pallet types, per-article pick instructions, tenant-defined attribute definitions with generated forms and list columns.
3. **Inventory + inbound**: movement ledger, balances, receipts, putaway tasks with smart suggestion.
4. **Outbound**: orders, allocation, pick tasks on the screen PWA, pack, ship confirmation, pick-check, deviations (model and rules under "Outbound model" below). Pick-by-voice is a later entitlement module on the same commands, not this sub-project.
...
- Commands, not entity diffs: `CreateLocationBatch(parent, type, count, pattern)`, `SetLocationDimensions(ids[], dims)`, `ConfirmPutaway(task_id, location_id, qty)`, `ConfirmPick(...)`, `CheckPick(task_id, verdict, note)`, `ReportDeviation(...)`. `CheckPick` is issued by the checker, not the picker; it is a new command type in sub-project 4 (`contracts/commands/CheckPick/v1.json`).
