...
1. **Layout & mapping**: warehouse creation with the right parameters, mobile mapping flow, web map view updating live as the floor maps, CSV/JSON import/export, badge unlock (PIN and device enrollment already shipped in sub-project 0), and the **label printing subsystem** (print agent, Bluetooth mobile printing, PDF fallback, location labels in walk order, user badges, constrained template editor; see Labels and barcode printing). **Lite (sellable POC, lands first):** warehouse with a code pattern, `Location` tree, `CreateLocationBatch` / `SetLocationDimensions`, floor mapping of one aisle, live schematic map in the back office. CSV/JSON, badge unlock and the whole printing subsystem wait for a later slice of this sub-project.
2. **Catalog**: articles, packaging levels (each/package/pallet), pallet types, per-article pick instructions, tenant-defined attribute definitions with generated forms and list columns. **Lite:** sku, name, one packaging level, seeded units. No attribute engine, no CSV.
3. **Inventory + inbound**: movement ledger, balances, receipts, putaway tasks with smart suggestion. **Lite:** receive one handling unit into `RECEIVING`, putaway task, `ConfirmPutaway`. Suggestion may be first empty mapped bin until the strategy interface lands. LPN is an on-screen code until printing ships.
4. **Outbound**: orders, allocation, pick tasks on the screen PWA, pack, ship confirmation, pick-check, deviations (model and rules under "Outbound model" below). Pick-by-voice is a later entitlement module on the same commands, not this sub-project. **Lite:** one manual order, allocate, pick on the floor PWA, ship-as-picked (`pack_step` optional). Pick-check and a required pack station wait.
5. **Billing**: usage snapshots, month-end invoicing, Fortnox sync, proration, dunning, internal admin. Must ship before the first paying customer's second month; trial handling and the hard cap are already in sub-project 0. Out of the first sellable demo.

Then pilot. Items 10-31 on the board build on the ledger, tasks and events without new foundations.

Entity order does not change: locations before articles before ledger before orders. Lite is a cut of each sub-project, not a shortcut around it. Cloud staging (SP0 G2/G3) is not required to start SP1 lite; the demo runs on local Aspire.
...
- **Integrations at the edge**: webhooks and events; partners never write stock. Headless **command contracts** are the same path our floor PWA uses, so a customer or partner scanner app (including a later voice client) can plug in without a second API.
...
- **Screen-first picking**: the pick client we recommend, and the one we build in outbound, is the floor PWA on a screen (desktop pick station or rugged scanner). Pick-by-voice is a sellable module, not the default.
...
| Catalog | Articles, packaging, pick instructions, attribute templates | 2 |
| Inbound + inventory | Receive, putaway, ledger, balances | 3 |
| Outbound | Orders, pick, ship, pick-check, deviations | 4 |
| Billing | Usage, invoices, Fortnox, dunning | 5 |
| Pick-by-voice | Voice headset client on the same `ConfirmPick` commands | later module |

Capabilities light up per tenant as entitlement flags on the subscription (platform-owned). Missing capabilities return **batch holds** with a clear reason — never half-working screens. We do not sell yard, wave planning, or full LMS in the pilot; that is the deliberate non-goal versus lumbering suites. The voice entitlement gates **our voice client**, not `ConfirmPick` from the screen PWA or a partner app.

### Product promises to keep sharp

- **Time-to-first-pick**: named path — enroll device, then map one aisle, then receive one handling unit, then putaway, then pick on a screen, then ship handoff — measurable in days, not a multi-month programme.
...
- **Customer-visible ops**: sync health (queue depth, last success, SSE), open deviations by age, “what’s stuck on this device”, and the live pick-check view (who is picking what, current line) are first-class surfaces — trust against black-box enterprise suites.
- **Headless by default**: OpenAPI + versioned command/event contracts; our Vue floor app is the reference client, not the only client. A voice-headset vendor speaks the same commands.
- **Advise screen picking**: sales tells customers to run pick work on a computer or scanner screen so quantity breakdown, per-article pick instructions and the pick-check view are visible. Voice remains available as a module for the customer who insists.

### Non-goals (say no in sales)

Yard management, wave/waveless orchestration as a product line, **full labour management** (standards, engineered labour, productivity scoring), custom on-prem plugins inside wms-core, and rebuilding ERP/MES inside Lagerkraft. Extend via commands, webhooks, and integrations — not by forking the core. Pick-check (one person confirming that pickers stacked and picked correctly) is an outbound gate, not LMS.
...
- **Location tree instead of fixed Sektion/Hylla/Hyllrad/Hyllplats tables.** `Location(warehouse_id, parent_id, type, code, path ltree, height_mm, width_mm, depth_mm, max_weight_g, barcode, status)` where `type` is zone/aisle/rack/level/bin/floor/dock/staging. Same mapping flow, but docks, floor spots and staging areas fit, and pallet racks versus shelving are just parameters. Default code pattern `{aisle}-{rack:2}-{level:2}-{bin:2}` (live preview `A-01-03-02`). Numeric tokens declare a width and are zero-padded; the server rejects any **bin** (or other leaf) code that does not match the full warehouse pattern, so `A-1-3-2` and `A-01-03-02` can never coexist. Parent codes are the pattern prefix up to that type (`A`, `A-01`, `A-01-03`). `Location.barcode` equals `code` until labels ship. `Location.aliases[]` (code, expires_at) keeps an old code scannable for 90 days after a label is replaced; unused until the printing slice. UI copy (sv): gång / hylla / hyllplan / plats for aisle / rack / level / bin. Hyllplatsaction becomes the general `change_log`.
...
- **Task** as the floor app's single work object (putaway, pick, move, count) with status, assignee, lines and a suggested location. Sub-project 0 creates `Task` and `TaskLine` (columns below, **no foreign keys** until article and location exist) so claim-work has a real table; nothing fills lines until outbound. Sub-project 4 adds pick-check columns on `Task` (`checked_by`, `checked_at`, `check_verdict` none|pending|passed|failed). **Deviation** (avvikelse) is raised when a command is rejected, a rule is overridden, or a worker reports a mismatch (`kind`: rejected | policy_override | over_receipt | short_pick | damage | count_mismatch | stale_command | reported | pick_check_fail).
- **Deviation model**: `Deviation(id client-generated, warehouse_id, kind, status open|pending_approval|resolved|dismissed, location_id?, handling_unit_id?, article_id?, qty_base?, command_id?, movement_id?, task_line_id?, reported_by, reported_at, detail jsonb, note, attachment_ids[], resolution, resolved_by, resolved_at)`. For a rejected command, **wms-core writes the deviation in the same transaction that stores the rejection in `processed_commands`**, with the original payload in `detail`; the device only renders it from the change feed. Resolution is a command, `ResolveDeviation(id, resolution, actions[])`, whose actions are existing commands (move to `QUARANTINE`, `write_off` with `inventory.adjust`, recount task, correction, dismiss with note); movements created this way carry `reference_type=deviation`. Rejected commands additionally offer **"apply with override"**, which re-runs the stored payload under the manager's permissions with a `policy_override` marker, so a putaway into a bin that was deactivated while the worker was offline is fixed in one click instead of re-keyed. Attachments: the device resizes photos to 1600 px (about 300 KB), keeps the blob in IndexedDB, the command carries client-generated `attachment_ids[]`, and blobs upload separately to Object Storage via presigned URLs when online (`Attachment(id, status pending|uploaded, object_key)`, 50 MB pending cap per device). Notifications per kind (none, immediate email, hourly digest; defaults immediate for `damage`, `pick_check_fail` and `pending_approval`) go through platform's email path. The device keeps a **"sync issues" list visible to any user on that device**, since the physical situation belongs to the place, not the person; workers also see "my reports". The dashboard shows open deviations by age and highlights anything older than 24 hours.
...
- **Fixed core columns** stay real columns because the WMS logic depends on them: `sku`, `name`, `gtin`, dimensions, weight, packaging levels, `pick_instruction`, `status`. These are never user-configurable. `pick_instruction` is nullable free text in the tenant's language, shown on every pick line; it does not drive allocation. Weight-sorted pick order is a later warehouse setting that uses existing weight columns, not an attribute.
...
- `StockBalance(location_id, article_id, handling_unit_id nullable, qty_base, reserved_qty_base, secondary_qty nullable)` in base units. Stock is not stored per packaging level; the level is a presentation and a quantity breakdown, not a stock-keeping key. Where case or pallet integrity matters (a sealed pallet that must ship as one), it is a `HandlingUnit` with `HandlingUnitContent(article_id, qty_base, packaging_level_id)` and the balance row points at it. Scanning the pallet's LPN moves its entire content in one movement per content row.
...
- **One pick task per order**, lines in **walk order** (location path); with `Warehouse.zone_picking` on, an order spanning zones becomes one task per zone. Wave and batch picking come later without changing `Task`. Walk order is how the picker moves. How goods go into the tote is a different question: v1 does **not** reorder lines by weight. Each line shows `Article.pick_instruction` so the picker can put heavy items at the bottom of a plyfa (plywood pallet crate) and fragile items on top. Later option, not v1: `Warehouse.pick_line_order = walk | heavy_first`, ranking by the picked level's `gross_weight_g` (fallback `Article.weight_per_base_unit_g`). Do not invent a second container type for plyfa.
- **The tote is a handling unit**: picked goods move bin → `PICKING` system location on a tote handling unit (client-generated id, LPN from the device's block). The tote is what the pack station scans. A plyfa, carton or plastic tote is the same entity with a different physical box.
- **Pick client is the screen PWA.** Sub-project 4 builds pick execution in `apps/floor` on a desktop pick station or a rugged scanner with a screen. Pick-by-voice is the later entitlement module in the capability map; it is the same `ConfirmPick` commands, not a second pipeline. We recommend the screen client to customers.
- **Pick-check** (the floor supervisor who watches pickers): `Warehouse.pick_check = off | advisory | required_before_pack` (default `off`). A user with `tasks.read_all` gets a live pick view (assignee, order, current line) on the floor app and in the back office, fed by the warehouse SSE stream. `CheckPick(task_id, verdict pass|fail, note)` requires `outbound.check` and may be recorded while the task is claimed or after it is done. Pass writes `Task.checked_by`, `checked_at`, `check_verdict=passed`. Fail writes `failed` and a `Deviation(kind=pick_check_fail)` on the task; **already-written pick movements stay** (the floor is the truth). `required_before_pack`: pack and ship commands reject with a not-ready reason until `check_verdict=passed`; that is a domain rejection of pack/ship, not a hold and not a rejection of the pick. `advisory` records the check and does not block. v1 the checker is `warehouse_manager` or `tenant_admin` (they hold `outbound.check`); a dedicated supervisor without manager powers is custom roles (v2). This is not labour management: no standards, no scoring, no engineered labour.
...
- Commands, not entity diffs: `CreateLocationBatch(warehouse_id, parent_id nullable, type, locations[{id, code, parent_id}])`, `SetLocationDimensions(ids[], height_mm, width_mm, depth_mm, max_weight_g)`, `ConfirmPutaway(task_id, location_id, qty)`, `ConfirmPick(...)`, `CheckPick(task_id, verdict, note)`, `ReportDeviation(...)`. The device turns mapping answers (which aisle, how many racks, levels per rack, bins per level) into codes and client-generated UUIDv7s; the server never mints location ids. Applied `details` may contain `id_map: [{from, to}]` when a code already existed (device rewrites queued commands that reference `from`). `CheckPick` is issued by the checker, not the picker; it is a new command type in sub-project 4 (`contracts/commands/CheckPick/v1.json`).
...
- **Floor app**: SSE entries are written straight into the Dexie stores, the same code path as the offline catch-up pull, so the UI is reactive through Dexie's `liveQuery` and Pinia. A picker sees a task reassigned or a bin's balance change within a second. A checker with `tasks.read_all` sees other pickers' in-progress pick tasks the same way. Optimistic local writes from their own commands are reconciled when the corresponding change_log entry (carrying the command id) arrives.
...
- Floor app adds Dexie, vite-plugin-pwa (Workbox precache + runtime cache), barcode scanning via `BarcodeDetector` with `@zxing/browser` fallback, keyboard-wedge input support so hardware scanners work unmodified, Web Bluetooth printing to Zebra mobile printers via `packages/labels` (printing slice of sub-project 1), large touch targets, one action per screen. Pick execution is this PWA on a computer or a scanner screen; `Article.pick_instruction` and `suggested_breakdown` share the confirm-pick screen. Users with `tasks.read_all` get a live pick list (the pick-check view) in addition to claiming their own work.
- **Sales UX (POC and after):** a buyer must understand the mapping flow and the live map without a narrator. Floor: one question per screen (gång, then hyllor, hyllplan, platser, same H/W/D?), a running preview of count and first/last code, confirm before the outbox write. Mapping starts from the floor home (**Kartlägg gång**) after a warehouse is picked by name; the onboarding wizard deep link is the full path, not a prerequisite. Office: the warehouse page **is** the live schematic (racks as columns, bins as cells) fed by the existing SSE path. When at least one warehouse exists, the schematic is the first thing on the page; create warehouse is a disclosure (**Nytt lager**) with name plus code pattern and a live example, extra fields behind **Mer**. Empty state (no warehouses) still shows the create form and names the next action. Strings stay in `packages/i18n`. No second component library. Devices, users and SSO stay secondary in the nav. Auth screens (login, signup, verify, tenant chooser) use a centered card; the office shell shows a wordmark, the current nav item, and the company name (never a raw tenant UUID).
...
5. **Map the first aisle**: hands off to the floor app with a deep link; completion is detected from the first `CreateLocationBatch` in the warehouse. The web map view updates live as the aisle appears, which is the moment the product clicks for most customers. Until the wizard ships, the same mapping flow is reached from the floor home (**Kartlägg gång**) after picking a warehouse.
...
- Outbound: `orders.manage` A, M; `outbound.pick` A, M, W; `outbound.pack` A, M, W; `outbound.ship` A, M, W; `outbound.check` A, M (record a pick-check verdict; live view is `tasks.read_all`).
...
Floor workers never hold a `*.read_all` or `*.manage` permission, which is what keeps the floor app's data footprint small: the sync snapshot for a worker contains the locations and articles of their warehouses and their own tasks. A user with `tasks.read_all` on the floor app also receives in-progress pick tasks for that warehouse (the live pick-check view).
...
## Labels and barcode printing (later slice of sub-project 1)

SP1 **lite** ships mapping and the live map **without** printing. This section remains the design for the printing slice that follows the sellable goods-in loop. Do not block SP1 lite on printers or the print agent.

Mapping without labels is half a job in production: the worker creates an aisle on the phone and should be sticking labels on it two minutes later, often with no Wi-Fi in that aisle. That constraint drives this slice when it lands.
...
- **Pallet and handling-unit labels (LPN)**: every pallet or box we track gets a `HandlingUnit` with a license plate. Tenants with a GS1 company prefix get a real **SSCC-18 in GS1-128** (AI 00); everyone else gets an internal `LP` + 12-digit sequence with a mod-10 check digit in Code 128. Formats are defined with the printing slice; SP3 lite may show the internal code on screen without printing. The receiving flow that creates handling units is sub-project 3.
...
- Buy two printers for the office before the **printing slice** starts (not before SP1 lite): a desktop Zebra ZD421 (203 dpi, network) and a mobile ZQ521 (Bluetooth), and add a hardware checklist to that slice's definition of done: mapping an aisle offline and printing its labels over Bluetooth, agent printing a 200-label batch with a paper-out mid-way and a resume, and an A4 sheet on a laser printer lining up with the grid.
...
3. Buy the two label printers (ZD421, ZQ521) before the printing slice, not before SP1 lite.
...
- 2026-09-19: office sales UX layout. When warehouses exist the schematic is first on the warehouse page; create is **Nytt lager**. Empty state still shows the form. Auth is a centered card; the shell shows company name, not a tenant UUID. Same `packages/ui`, no second library.
- 2026-09-19: sellable POC path. Sub-projects 1–4 may land as lite slices in entity order (locations, then articles, then ledger, then orders) without inventing a parallel stack. SP1 lite is warehouse + `Location` tree + floor mapping of one aisle + live schematic map; CSV, attributes, labels/print agent, badge unlock, onboarding wizard, pick-check, required pack and billing stay specified but deferred. `CreateLocationBatch` payload is `warehouse_id`, `parent_id`, `type`, and client-generated `locations[{id, code, parent_id}]` (device builds codes from mapping answers; server never mints location ids; applied `id_map` on code collision). Default pattern `{aisle}-{rack:2}-{level:2}-{bin:2}`; parent codes are pattern prefixes; bin codes must match the full pattern. Mapping entry without the wizard is floor home **Kartlägg gång**. Sales UX: one question per screen, preview then confirm, office map is the warehouse page. Printers are a gate for the printing slice, not SP1 lite. Cloud staging is not required to start.
- 2026-09-13: screen-first picking. The outbound pick client is the floor PWA on a computer or scanner screen; pick-by-voice is a later entitlement module on the same `ConfirmPick` commands (the entitlement gates our voice client, not ConfirmPick). v1 stacking guidance is `Article.pick_instruction` on the pick line; walk order stays location path; `Warehouse.pick_line_order = heavy_first` is a later option using existing weight columns. Pick-check (`Warehouse.pick_check`, live view via `tasks.read_all`, `CheckPick` + `outbound.check`, `Deviation(kind=pick_check_fail)`) is an outbound gate, not LMS. Fail does not unwind pick movements. `required_before_pack` rejects pack/ship until pass.
