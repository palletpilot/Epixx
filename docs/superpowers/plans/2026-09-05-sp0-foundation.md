...
- Ids are UUIDv7 (`Guid.CreateVersion7()`); timestamps are `timestamptz`; money is integer Ã¶re; quantities that exist as columns are `NUMERIC(18,6)` / decimal strings on the wire even when nothing fills them yet.
...
Seed: four system roles (`tenant_admin`, `warehouse_manager`, `floor_worker`, `viewer`) and the plans `start`, `pro`, `business` with placeholder prices (Ã¶re), `is_public` true for the first two.
...
### Task C1. Tenant DB context and migrate command (done 2026-09-12, 67e0809)
...
A1 â†’ A2 â†’ A3 are sequential and short. B1 and C1 can proceed in parallel once A2 is in; B2 to B7 depend on B1 (B7 after B2 is enough; it does not need B3); C2 to C4 depend on C1; D1 depends on C2, B2 and B5; D2 depends on C4 and D1; E1 depends on C4 and B1; F1 can start after A2 (empty `contracts/` trees exist from A1; real schemas arrive in C3/C4); F2 depends on B2, B3, B7, D2; F3 depends on D1, D2, B5; G1 can start after A3 and grows with every task; G2 and G3 are last and mostly needs-human.
